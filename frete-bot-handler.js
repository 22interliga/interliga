...
async function handleFreteMessage(sock, telefoneRaw, textoRaw, db) {
  ...
  if (session && session.step === 'aguardando_regiao') {
    ...
    await enviarTexto(
      sock,
      telefone,
      `Pronto! Você está marcado como DISPONÍVEL para fretes em direção a: ${regiao}.\n\nAvisaremos por aqui quando surgir uma carga compatível.`
    );
    return true;
  }

  // --- 3. Aguardando resposta de DISPONIBILIDADE ---
  ...
  // --- 4. Sem sessão ativa: qualquer mensagem inicia o fluxo de disponibilidade ---
