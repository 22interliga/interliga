...
async function aceitarCorrida() {
  alert('[DEBUG 1] aceitarCorrida() foi chamada');
  clearInterval(state.countdownInterval);
  clearInterval(state.somRepeticaoInterval);
  const corrida = state.corridaAtual;
  if (!corrida) {
    alert('[DEBUG] state.corridaAtual está vazio! Corrida não disponível.');
  ...
  alert('[DEBUG 2] Corrida existe. ID=' + state.corridaAtualId + ' firebaseReady=' + firebaseReady);
  ...
  if (firebaseReady && db && state.corridaAtualId && !String(state.corridaAtualId).startsWith('local-')) {
    try {
      ...
      alert('[DEBUG 3] Firebase atualizado com sucesso!');
    } catch (e) {
      alert('[DEBUG ERRO] Falha ao atualizar Firebase: ' + e.message);
    }
  } else {
    alert('[DEBUG] NÃO atualizou Firebase (condição falsa). db=' + !!db + ' corridaAtualId=' + state.corridaAtualId);
  }
  ...
  alert('[DEBUG 4] Prestes a chamar go("screen-ongoing")');
  try {
    go('screen-ongoing');
    alert('[DEBUG 5] go() executada sem erro!');
  } catch (e) {
    alert('[DEBUG ERRO CRÍTICO] go() falhou: ' + e.message + '\n\n' + e.stack);
  }
}
...
function onEnterOngoing() {
  alert('[DEBUG 6] onEnterOngoing() foi chamada!');
  const corrida = state.corridaAtual;
  if (!corrida) { alert('[DEBUG] onEnterOngoing: sem corrida atual!'); return; }
...
function iniciarBroadcastPosicao() {
  ...
  watchPositionId = navigator.geolocation.watchPosition(
    ...
    (erro) => console.warn('[motorista] erro ao obter posição:', erro),
