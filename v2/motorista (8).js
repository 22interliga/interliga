...
async function aceitarCorrida() {
  ...
  console.log('[motorista] aceitando corrida. ID:', state.corridaAtualId, 'firebaseReady:', firebaseReady);
  if (firebaseReady && db && state.corridaAtualId && !String(state.corridaAtualId).startsWith('local-')) {
    try {
      ...
      console.log('[motorista] status atualizado para "aceita" no Firebase com sucesso');
    } catch (e) { console.error('[motorista] Erro ao aceitar corrida:', e); }
  } else {
    console.warn('[motorista] NÃO atualizou Firebase. firebaseReady:', firebaseReady, 'db:', !!db, 'corridaAtualId:', state.corridaAtualId);
  }
