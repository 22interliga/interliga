...
function initHomeMapDriver() {
  if (homeMapDriver) { setTimeout(() => homeMapDriver.invalidateSize(), 100); return; }
  ...
  const tryInit = () => {
    ...
    navigator.geolocation?.getCurrentPosition((pos) => {
      ...
      L.circleMarker([latitude, longitude], { radius: 8, color: '#1E7A2E', fillColor: '#1E7A2E', fillOpacity: 0.8 }).addTo(homeMapDriver);
    }, () => {}, { timeout: 5000 });
  };
  tryInit();
}
...
function onEnterOngoing() {
  ...
  document.getElementById('ongoing-origem').textContent = corrida.origem;
  document.getElementById('ongoing-destino').textContent = corrida.destino;
  document.getElementById('passenger-name').textContent = 'Passageiro';
  document.getElementById('passenger-avatar').textContent = 'PS';
  document.getElementById('passenger-rating').textContent = '⭐ 4.9';

  chegouAoCliente = false;
  document.getElementById('btn-cheguei').hidden = false;
  document.getElementById('btn-finalizar-corrida').hidden = true;
