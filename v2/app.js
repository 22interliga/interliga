...
function attachAddressAutocomplete(inputEl, onSelect) {
  const container = inputEl.closest('.form-card');
  const suggestionsBox = container?.querySelector('.address-suggestions');
  ...
  document.addEventListener('click', (e) => {
    if (!container.contains(e.target)) suggestionsBox.classList.remove('is-open');
  });
}
...
function ouvirAceiteCorrida(corridaId) {
  ...
  state.corridaListenerUnsub = onSnapshot(doc(db, 'corridas', corridaId), (snap) => {
...
function iniciarChatCorrida() {
  ...
  const q = query(
    collection(db, 'corridas', state.corridaId, 'mensagens'),
    orderBy('ts', 'asc'), limit(50)
  );
  state.chatListenerUnsub = onSnapshot(q, (snap) => {
...
async function enviarMensagemChat(texto) {
  ...
  if (firebaseReady && db && state.corridaId) {
    try {
      await addDoc(collection(db, 'corridas', state.corridaId, 'mensagens'), {
        texto, de: 'passageiro', ts: serverTimestamp(),
      });
...
document.getElementById('btn-call-driver')?.addEventListener('click', () => {
  showToast('📞 Pedido de ligação enviado ao motorista');
});
...
document.getElementById('btn-cancelar-corrida')?.addEventListener('click', () => {
  ...
  if (firebaseReady && db && state.corridaId && !state.corridaId.startsWith('local-')) {
    updateDoc(doc(db, 'corridas', state.corridaId), { status: 'cancelada' }).catch(() => {});
  }
...
document.getElementById('btn-confirmar-agendamento')?.addEventListener('click', () => {
  const origem = document.getElementById('sched-origem').value.trim();
  const destino = document.getElementById('sched-destino').value.trim();

  if (!selectedDay) { showToast('⚠️ Selecione uma data'); return; }
  if (!selectedSlot) { showToast('⚠️ Selecione um horário'); return; }
  if (!origem) { showToast('⚠️ Informe o endereço de embarque'); return; }
  if (!destino) { showToast('⚠️ Informe o destino'); return; }
  ...
  setTimeout(() => go('screen-home'), 1200);
});
